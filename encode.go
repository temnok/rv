...
func encodeR(f7, rs2, rs1, f3, rd, op int32) int32 {
	return f7<<25 | rs2<<20 | rs1<<15 | f3<<12 | rd<<7 | op<<2 | 3
}

func encodeI(imm, rs1, f3, rd, op int32) int32 {
	return imm<<20 | rs1<<15 | f3<<12 | rd<<7 | op<<2 | 3
}

func encodeS(imm, rs2, rs1, f3, op int32) int32 {
	a := bits(imm, 5, 7)
	b := bits(imm, 0, 5)
	return a<<25 | rs2<<20 | rs1<<15 | f3<<12 | b<<7 | op<<2 | 3
}

func encodeB(imm, rs2, rs1, f3, op int32) int32 {
	a := bit(imm, 12)
	b := bit(imm, 11)
	c := bits(imm, 5, 6)
	d := bits(imm, 1, 4)
...
func encodeU(imm, rd, op int32) int32 {
	return imm<<12 | rd<<7 | op<<2 | 3
}

func encodeJ(imm, rd, op int32) int32 {
	a := bit(imm, 20)
	b := bits(imm, 12, 8)
	c := bit(imm, 11)
	d := bits(imm, 1, 10)
