...
func sv32(cpu *state.CPU, virtAddr int, physAddr *int, access int) {
	...
	*physAddr = bi.Ts(pte, 10, 20)<<12 | bi.Ts(virtAddr, 0, shift)
}
...
func loadPTEsv32(cpu *state.CPU, virtAddr int, targetPTE, shift *int) {
	*targetPTE = 0
	...
	pteAddr := bi.Ts(cpu.CSR.Satp, 0, 20)<<12 | bi.Ts(virtAddr, 22, 10)<<2
	...
	*shift = 22
	...
	if !isLeaf {
		pteAddr = bi.Ts(pte, 10, 20)<<12 | bi.Ts(virtAddr, 12, 10)<<2
		if !cpu.Bus.Read(pteAddr, &pte, 4) {
			trap.Enter(cpu, trap.LoadAccessFault, virtAddr)
			return
		}
